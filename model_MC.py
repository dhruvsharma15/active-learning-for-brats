...
class Unet_model(object):
    ...
    def level_block(self,m, dim, depth, inc, acti, do, bn, up,format_="channels_last"):
        if depth > 0:
            ...
            n=concatenate([n,m])
            #the decoding path
            m = self.res_block_dec(n, do,dim, acti, bn, format_)
        else:
            m = self.res_block_enc(m, do,dim, acti, bn, format_)
        return m
    ...
    def res_block_enc(self,m, drpout,dim,acti, bn,format_="channels_last"):
        ...
        n=Dropout(drpout)(n, training = True)
...
if __name__ == '__main__':
    ...
    plot_model(model=model, to_file='../graph_MC.png')
    print('success')
